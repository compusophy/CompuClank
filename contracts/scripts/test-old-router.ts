...
// Actions - try the OLD values that were in SwapFacet originally
const SWAP_EXACT_IN_SINGLE = 0x06;
const SETTLE_ALL = 0x0b; // Old value
const TAKE_ALL = 0x0e; // Old value

async function main() {
  const [signer] = await ethers.getSigners();
  console.log(`Testing with OLD router: ${UNIVERSAL_ROUTER}`);
  ...
  // Try OLD action values
  console.log(`Using OLD action values: SETTLE_ALL=0x0b, TAKE_ALL=0x0e`);
  ...
  console.log("Testing swap with OLD router...");
  ...
  // Also try with NEW action values
  console.log("\n\nTrying with NEW action values: SETTLE_ALL=0x0C, TAKE_ALL=0x0F");
  
  const actions2 = ethers.solidityPacked(
    ["uint8", "uint8", "uint8"],
    [0x06, 0x0C, 0x0F]
  );
  
  const param0_2 = abiCoder.encode(
    ["tuple(tuple(address,address,uint24,int24,address),bool,uint128,uint128,bytes)"],
    [[
      [poolKey.currency0, poolKey.currency1, poolKey.fee, poolKey.tickSpacing, poolKey.hooks],
      zeroForOne,
  ...
  const params2 = [param0_2, param1, param2];
  const input1_2 = abiCoder.encode(["bytes", "bytes[]"], [actions2, params2]);
  
  try {
    const tx = await router.execute(commands, [input0, input1_2, input2], deadline, {
      value: amountIn,
      gasLimit: 500000
    });
    console.log(`Transaction sent: ${tx.hash}`);
    const receipt = await tx.wait();
    console.log(`✅ SUCCESS with NEW actions! Gas used: ${receipt.gasUsed}`);
  } catch (e: any) {
    console.log(`❌ FAILED with NEW actions`);
    console.log(`Error: ${e.message?.slice(0, 300)}`);
    if (e.data) console.log(`Error data: ${e.data}`);
  }
}
