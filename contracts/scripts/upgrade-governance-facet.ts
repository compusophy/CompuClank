...
async function main() {
  ...
  console.log("Upgrading GovernanceFacet with account:", signer.address);
  ...
  const deployTx = governanceFacet.deploymentTransaction();
  if (deployTx) {
    console.log("   Deploy TX:", deployTx.hash);
    await deployTx.wait(2); // Wait for 2 confirmations
  }
  const governanceFacetAddress = await governanceFacet.getAddress();
  console.log("   New GovernanceFacet:", governanceFacetAddress);
  
  // Verify contract has code
  const code = await ethers.provider.getCode(governanceFacetAddress);
  ...
  console.log("   Selectors:", governanceSelectors.length);
  ...
  for (const sel of governanceSelectors) {
  ...
  await tx.wait();
  console.log("   ✅ GovernanceFacet upgraded!");
}
